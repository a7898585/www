...
class Httpsqs {
    ...
    public function __construct($host = '117.25.133.71', $port = 1226, $auth = 'hic3F5R', $charset = 'utf-8') {
    ...
    public function put($queue_name, $queue_data) {
        $result = $this->http_post("/?auth=" . $this->httpsqs_auth . "&charset=" . $this->httpsqs_charset . "&name=" . $queue_name . "&check=BSC258&opt=status", $queue_data);
    ...
    public function get($queue_name) {
        ...
        //	var_dump($result);
        //echo "/?auth=".$this->httpsqs_auth."&charset=".$this->httpsqs_charset."&name=".$queue_name."&opt=get";
    ...
    public function view($queue_name, $pos) {
        $result = $this->http_get("/?auth=" . $this->httpsqs_auth . "&charset=" . $this->httpsqs_charset . "&name=" . $queue_name . "&opt=view&check=BSC258&pos=" . $pos);
        if ($result == false || $result["data"] == "HTTPSQS_ERROR" || $result["data"] == false) {
            return false;
        }
        return $result["data"];
