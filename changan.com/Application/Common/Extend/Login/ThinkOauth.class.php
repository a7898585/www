...
abstract class ThinkOauth{
    ...
    public function __construct($token = null){
        ...
        $this->Type = strtoupper(array_pop($class));
        //获取应用配置
    ...
    public static function getInstance($type, $token = null) {
        $driver = 'Common.Extend.Login.Driver.'.ucfirst(strtolower($type));
        import($driver);
        $namespace = str_replace('.', '\\', $driver);
        if (class_exists($namespace)) {
            return new $namespace($token);
        } else {
            E(L('_CLASS_NOT_EXIST_') . ':' . $namespace);
        }
    }
