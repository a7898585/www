...
class FriendModel extends MY_Model {
    ...
    public function follow($param) {
        $type = 'U110';
        error_log('model' . date("Y-m-d H:i:s") . print_r($param, true), 3, '/home/httpd/logs/addblack.log');
        $rs = $this->socket['passport']->senddata($type, $param);
        $rs = xmltoarray($rs);
        error_log('U110xml' . date("Y-m-d H:i:s") . print_r($rs, true), 3, '/home/httpd/logs/addblack.log');
        ...
        if (ISCACHE) {
            ...
            if ($this->_checkrs($rs)) {
                if ($data['StartNo'] == '-1') {
                    ...
                    $rs['Records'] = ($rs['Records'] == 1) ? array('0' => $rs['Records']) : $rs['Records'];
                }
