...
class SimpleCaptcha {
	...
	public function validate($input, $uid)
    {
		session_start();
		$return = false;
        if(isset($_SESSION[$this->session_var]))
		{
			$AuthCode = $_SESSION[$this->session_var];
			unset($_SESSION[$this->session_var]);
			$return = (strcmp($AuthCode, md5(strtolower($input))) == 0);
		}
		else if(isset($_COOKIE[$this->session_var]))
		{
			$AuthCode = $_COOKIE[$this->session_var];
			setcookie($this->session_var, '', time()-300, "/", ".cnfol.com");
			$return = (strcmp($AuthCode, md5(strtolower($input))) == 0);
		}
		else if(class_exists('Memcache'))
		{
			$mcache   = &load_class('Memcache');
			$AuthCode = $mcache->get($this->session_var.$uid);
			$mcache->delete($this->session_var.$uid);
			$return = (strcmp($AuthCode, md5(strtolower($input))) == 0);
		}
		return $return;
    }

    public function CreateImage($uid=0) 
	{
        $ini = microtime(true);
		...
		$codestr = md5(strtolower($text));
		
		session_start();
        $_SESSION[$this->session_var] = $codestr;
		
		setcookie($this->session_var, $codestr, time()+300, "/", ".cnfol.com");
		
		if(class_exists('Memcache')){
			$mcache   = &load_class('Memcache');
			$mcache->set($this->session_var.$uid, $codestr, time()+300);
		}
    ...
    protected function GetRandomCaptchaText($length = null) {
        ...
        for ($i=0; $i<$length; $i++) {
            if ($vocal) {
                ...
                $text .= substr($words, mt_rand(0, 25), 1);
            }
            $vocal = !$vocal;
        }
    ...
    protected function WriteText($text, $fontcfg = array()) {
        ...
        for ($i=0; $i<$length; $i++) {
			...
			if($this->minSize && $this->maxSize){
				$fontsize = rand($this->minSize, $this->maxSize)*$this->scale*$fontSizefactor;
			}else{
