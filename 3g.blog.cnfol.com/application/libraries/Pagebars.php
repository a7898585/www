...
//        <span>当前第<i class="CoRed">1/15</i>页，跳转至第<input type="text" class="Inpjump">页</span>
        $pager_Links = "<span>当前第<i class=\"CoRed\"> " . $this->cur_page . "/" . $this->pages . "</i>页，跳转至第<input type=\"text\" class=\"Inpjump\"  onkeyDown=\"enterPress('" . $this->baseLink . $this->nsplit . "', event)\"   onkeypress=\"enterPress('" . $this->baseLink . $this->nsplit . "')\"    onblur=\"jump('" . $this->baseLink . $this->nsplit . "')\">页</span>";
    ...
    function upDownListAjax($id, $callback) {
        if ($this->pages < 2)
            return '';
        $pager_Links = '<div class="SelectPage">当前第<em>' . $this->cur_page . '/' . $this->pages . '</em>页，跳转至第<input type="text" class="Inpjump" onblur="' . $callback . '('. $id . ',this.value)"页</div>';
