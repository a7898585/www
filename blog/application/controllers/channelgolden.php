...
class Channelgolden extends MY_Controller {

    function Channelgolden() {
    ...
    function Index() {
        ...
        $data['header'] = $blocks['channalgoldenhead'];
        $data['footer'] = $blocks['channalgoldenfoot'];
        $data['shtml'] = $this->config->item('shtml_path');
        $data['baseurl'] = $this->config->item('base_url');
        $this->load->view('/channal/channal_golden_index.shtml', $data);
    }
    ...
    function duokong() {
        ...
        $this->load->model('channel_socket');
        $TolCnt = $this->channel_socket->duoKong($sign);
        echo json_encode($TolCnt);
    }
    ...
    function sendWeiBo() {
        ...
        $url = config_item('weiboapi_url');
        $source = 'i.blog.cnfol.com';
        $content = utf8_str($this->input->get('content'), 200);
        $uid = $this->input->get('uid');

        $result = curl_post($url, array('content' => htmlspecialchars($content), 'source' => htmlspecialchars('<a href=' . config_item("base_url") . ' target="_blank">' . $source . '</a>'), 'uid' => $uid));
        echo $result;
    }
    ...
    function getWeiBo() {
        ...
        $this->load->model('channel_socket');
        $url = "http://t.cnfol.com/iframe/GetNewWeibo/13?r=" . time();
        $result = $this->channel_socket->getWeiBo($url);
        echo $result;
    }
    ...
    function getNewestWeiBo() {
        ...
        $url = "http://t.cnfol.com/iframe/GetNewEachWeibo?r=" . time();
