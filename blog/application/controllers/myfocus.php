...
//		
    ...
    function domainInfo($domainname) {
        ...
        $isowner = $this->_checkOwnUser($bloginfo['UserID']); //标志是否有管理权限
        ...
        if (isset($isowner) && $isowner == true) {
            ...
            $isFrends = $this->_isFriend($this->user['userid'], $bloginfo['UserID']);
            if ($isFrends == 0 || $isFrends == 2) {
                if ($isFrends != '') {
                    $ext = '<a class = "Plus" href = "javascript:delAttention(\'' . $bloginfo['UserID'] . '\',\'add\') ;" >加关注</a>';
                } else {
                    $url = $this->config->item('base_url') . '/index.php/widget/login';
                    $ext = '<a class = "Plus" href = "javascript:;" onclick = "showiframe(\'iframeSrc\',\'popupIframe\',\'' . $url . '\',\'\',\'\');">加关注</a>';
                }
            } else {
                $ext = '<a class = "HasPlus" href = "javascript:delAttention(\'' . $bloginfo['UserID'] . '\',\'del\');">取消关注</a>';
            }
            $addStr = '<div class="NmSrBox">' . $bloginfo['NickName'] . '</div><div class="focus">' . $ext . '</div>';
        }
        $str = '
		<div class="PicBox"><img src="' . $head . '" /></div>' . $addStr . '
