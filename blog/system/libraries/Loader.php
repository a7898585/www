...
class CI_Loader {
    ...
    function cnfol_database($db_group='test',$db_connect=true)
    {
        // Grab the super object
        $CI =& get_instance();
        //Êý¾Ý¿âÅäÖÃ
        ...
        $db_params = $db[$db_group];
        //load database class
        require_once(BASEPATH.'libraries/Mysqli'.EXT);
        $CI->db = '';
        $CI->db = & new CI_Mysqli($db_params);
        if($db_connect === true)
        {
            $CI->db->connect();
        }
