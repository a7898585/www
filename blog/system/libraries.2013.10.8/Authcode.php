...

{
    ...
    //默认字体文件
    var $fontfile = '/home/html/cnfol_framework/CI/system/fonts/arial.ttf';
    /**
     * 图像信息
     *  type:   图像类型
     *  mime:   MIME 类型
    ...
    function CI_Authcode()
    {
        ...
        $this->image = array('type'=>'png', 'mime'=>$infomation['mime'],
                             ...
                             'height'=>20);
        //随机的位置
        $tmpleft =mt_rand(1,15);
        $tmptop  =mt_rand(($this->image['height']*0.5),($this->image['height']));
        $this->font = array('space'=>5, 'size'=>12, 'left'=>$tmpleft,
                            'top'=>$tmptop,
                            'file'=>$this->fontfile);
    ...
    function paint($filename='')
    {
        ...
        // 如果 session 未启用, 则开启它
        session_start();
        $_SESSION[$this->session] = md5(strtolower($the_code));
        $_SESSION[$this->session.'_none_case'] = md5(strtolower($the_code));
        
        setcookie($this->session, md5(strtolower($the_code)), time()+300, "/", ".cnfol.com");
        setcookie($this->session.'_none_case', md5(strtolower($the_code)), time()+3600, "/", ".cnfol.com");

		$mcache   = &load_class('Memcache');
		$mcache->set($this->session.'_none_case', md5(strtolower($the_code)), time()+3600);
		
        // 绘制图像干扰
        $this->paintMolestation($im);
        //imagestring($im, 3, 3, 1, $the_code,'');
        // 输出
    ...
    function validate($input)
    {
		...
		{
			$AuthCode = $_SESSION[$this->session];
			unset($_SESSION[$this->session]);
			$return = (strcmp($AuthCode, md5(strtolower($input))) == 0);
		}
		else if(isset($_COOKIE[$this->session]))
		{
			$AuthCode = $_COOKIE[$this->session];
			setcookie($this->session, '', time()-3600, "/", ".cnfol.com");
			$return = (strcmp($AuthCode, md5(strtolower($input))) == 0);
		}else{
			$mcache   = &load_class('Memcache');
			$AuthCode = $mcache->get($this->session.'_none_case');
			$return = (strcmp($AuthCode, md5(strtolower($input))) == 0);
		}
		return $return;
    }
    ...
    function generateCode()
    {
		...
		if (!isset($this->code['characters']) || empty($this->code['characters'])) {
			return false;
		}
        ...
        while ($i < $this->code['length'])
        {
            ...
            {
                $code[$i]['color']['r'] = mt_rand(10, 100);
                $code[$i]['color']['g'] = mt_rand(10, 100);
                $code[$i]['color']['b'] = mt_rand(10, 100);
            } else
            {
