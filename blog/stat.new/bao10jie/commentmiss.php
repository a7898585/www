...
//没有要处理的记录直接退出
if (!file_exists($config['commentidmiss'])) {
    echo 'no date';
    exit(-1);
}

$commentidArray = file_get_contents($config['commentidmiss']);
...
foreach ($commentidArray as $ArticleCommentID) {
    if (trim($ArticleCommentID) == '') {
        continue;
    }
    ...
    error_log($ArticleCommentID . '|' . print_r($tres, true) . '|', 3, '/home/www/html/logs/articlecommentidfalsebaologmissagin_' . date('Ymd') . '.log');
    if (!$tres) {
        error_log($ArticleCommentID . ',', 3, '/home/www/html/logs/commentidmissagin_' . date('Ymd') . '.log');
        continue;
    }

    foreach ($tres as $key => $value) {

        error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog1missagin_' . date('Ymd') . '.log');
        ...
        if ($result['error'] == 1) {
            ...
            if ($result->flag == 1) {
                ...
                error_log(print_r($rs, true) . '|' . print_r($param, true) . '|', 3, '/home/www/html/logs/articlecommentbaogwlogmissagin_' . date('Ymd') . '.log');
                ...
                error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog2missagin_' . date('Ymd') . '.log');
                ...
                error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog4missagin_' . date('Ymd') . '.log');
            }

            //echo($value['CommentID'].'|'.$result->flag.'|'.$results.'<br>');
            error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog8missagin_' . date('Ymd') . '.log');
            ...
            error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog5missagin_' . date('Ymd') . '.log');
            ...
            error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog6missagin_' . date('Ymd') . '.log');
        } else {
            $params['CommentID'] = $value['CommentID'];
            $params['IllegalArticles'] = 1;
            ...
            error_log($value['CommentID'] . '|', 3, '/home/www/html/logs/articlecommentbaolog7missagin_' . date('Ymd') . '.log');
        }
    }
}

echo('ok');
unlink($config['commentidmiss']);
