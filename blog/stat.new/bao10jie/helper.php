...
function modifyGetInfo($ArtIds) {
    ...
    $rowid = $db->get_values("select TagID
				from tbBlogTagArticle where ArticleID={$ArtIds[0]}");
    $tagids = '';
    if (!empty($rowid)) {
    ...
    $tagnames = $db->get_values("select Name from tbBlogTag where  TagID in(" . $tagids . ")");
...
function delBlogArticleList($data) {
    ...
    $flag = $memcache->delete($ckey);
    //error_log($flag.'|'.$ckey.'|',3,'/home/www/html/logs/alog.log');

    $ckey = $config['K1040'];
    $ckey = str_replace('{MemberID}', $data['MemberID'], $ckey);
    $ckey = str_replace('{SelfRecommend}', '-1', $ckey);
    $flag2 = $memcache->delete($ckey);
    //error_log($flag2.'|'.$ckey.'|',3,'/home/www/html/logs/alog.log');

    $ckey = $config['K1005'];
    $ckey = str_replace('{MemberID}', $data['MemberID'], $ckey);
    $flag3 = $memcache->delete($ckey);
    //error_log($flag3.'|'.$ckey.'|',3,'/home/www/html/logs/alog.log');

    $ckey = $config['K1023'];
    $ckey = str_replace('{MemberID}', $data['MemberID'], $ckey);
    $flag4 = $memcache->delete($ckey);
    //error_log($flag4.'|'.$ckey.'|',3,'/home/www/html/logs/alog.log');

    $flag5 = delBlogUserTagArticle($data);

    //error_log($flag.'|'.$flag2.'|'.$flag3.'|'.$flag4.'|'.$flag5,3,'/home/www/html/logs/alog.log');
