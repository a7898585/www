...
//没有要处理的记录直接退出
if (!file_exists($config['articleidmiss'])) {
    echo 'no date';
    exit(-1);
}

$articleidArray = file_get_contents($config['articleidmiss']);

$articleidArray = explode(',', $articleidArray);
...
foreach ($articleidArray as $ArticleID) {
    ...
    error_log($ArticleID . '|' . print_r($tres, true) . '|', 3, '/home/www/html/logs/articleidfalsebaologmissagin_' . date('Ymd') . '.log');


    if (!$tres) {
        error_log($ArticleID . ',', 3, '/home/www/html/logs/articleidmissagin_' . date('Ymd') . '.log');
        continue;
    }
    ...
    foreach ($tres as $key => $value) {
        ...
        error_log($value['ArticleID'] . '|', 3, '/home/www/html/logs/articlebaolog1missagin_' . date('Ymd') . '.log');
        ...
        if ($result['error'] == 1) {
            ...
            if ($result->flag == 1) {
                ...
                error_log($value['ArticleID'] . '|', 3, '/home/www/html/logs/articlebaolog2missagin_' . date('Ymd') . '.log');
                ...
                error_log($value['ArticleID'] . '|', 3, '/home/www/html/logs/articlebaolog4missagin_' . date('Ymd') . '.log');
            }
            ...
            error_log($value['ArticleID'] . '|', 3, '/home/www/html/logs/articlebaolog5missagin_' . date('Ymd') . '.log');
            ...
            //error_log($result['error'].'|',3,'/home/www/html/logs/articlebaolog.log');
            error_log($value['ArticleID'] . '|', 3, '/home/www/html/logs/articlebaolog6missagin_' . date('Ymd') . '.log');
        } else {
            $param['MemberID'] = $value['MemberID'];
            $param['ArticleID'] = $value['ArticleID'];
            $param['IllegalArticles'] = 1;
            ...
            //error_log($result['error'].'|',3,'/home/www/html/logs/articlebaolog.log');
            error_log($value['ArticleID'] . '|', 3, '/home/www/html/logs/articlebaolog7missagin_' . date('Ymd') . '.log');
        }
    }
}

echo('ok');
unlink($config['articleidmiss']);
