...
class content_form {
	...
	function get($data = array()) {
		...
		foreach($this->fields as $field=>$v) {
			if(defined('IN_ADMIN')) {
				if($v['iscore'] || $v['isbase']==2 || check_in($_SESSION['roleid'], $v['unsetroleids'])) continue;
			} else {
	...
	function texts($field, $value, $fieldinfo) {
		...
		var str = "<li id=\'textsfile"+ids+"\'><input type=\'text\' name=\'"+returnid+"_fileurl[]\' value=\'\' style=\'width:160px;\' class=\'input-text\'> <input type=\'text\' name=\'"+returnid+"_filename[]\' value=\'¸½¼þËµÃ÷\' style=\'width:160px;\' class=\'input-text\'> <input type=\'text\' name=\'"+returnid+"_filename2[]\' value=\'¸½¼þËµÃ÷\' style=\'width:160px;\' class=\'input-text\'> <a href=\"javascript:remove_div(\'textsfile"+ids+"\')\">ÒÆ³ý</a> </li>";
		$(\'#\'+returnid).append(str);
		 }</script>';
	...
	function textarea($field, $value, $fieldinfo) {
		...
		$str = "<textarea name='info[{$field}]' id='$field' style='width:{$width}%;height:{$height}px;' $formattribute $css;if($maxlength) onkeyup=\"strlen_verify(this, '{$field}_len', {$maxlength})\";>{$value}</textarea>";
	...
	window.top.art.dialog({title:'".L('cut_the_picture','','content')."', id:'crop', iframe:'index.php?m=content&c=content&a=public_crop&module=content&modelid='+modelid+'&picurl='+encodeURIComponent(id)+'&input=$field&preview=".($show_type && defined('IN_ADMIN') ? $field."_preview" : '')."', width:'680px', height:'480px'}, 	function(){var d = window.top.art.dialog({id:'crop'}).data.iframe;
	...
	function typeid($field, $value, $fieldinfo) {
		...
		foreach($type_data as $_key=>$_value) {
			if(in_array($_key,$usable_array)) $data[$_key] = $_value['name'];  // && $_value['parentid']==$setting['sortid']
		}
