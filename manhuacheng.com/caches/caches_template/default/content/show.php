...
<div class="main">
	<div class="col-left">
    	<div class="crumbs"><a href="<?php echo siteurl();?>">首页</a><span> &gt; </span><?php echo catpos($catid);?> 正文</div>
        <div id="Article">
        	<h1><?php echo $title;?><br />
<span><?php echo $inputtime;?>&nbsp;&nbsp;&nbsp;来源：<?php echo $copyfrom;?>&nbsp;&nbsp;&nbsp;评论：<a href="#comment_iframe" id="comment">0</a> 点击：</span><span id="hits"><?php echo $views;?></span></h1>
			<div class="content">
...
<?php } ?>
			<div id="pages" class="text-c"><?php echo $pages['lists'];?></div>
            ...
            <strong>相关热词搜索：</strong><?php $n=1;if(is_array($keywords)) foreach($keywords AS $keyword) { ?><a href="<?php echo APP_PATH;?>index.php?m=content&c=tag&catid=<?php echo $catid;?>&tag=<?php echo urlencode($keyword);?>" class="blue"><?php echo $keyword;?></a> 	<?php $n++;}unset($n); ?>
             ...
             <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=bb7c1f39bebf33c971022ae966402a2f&action=previous_next&id=%24id&modelid=1&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'previous_next')) {$data = $content_tag->previous_next(array('id'=>$id,'modelid'=>'1','limit'=>'20',));}?>
            <p class="f14">
                <p style="float:left"><strong>上一篇：</strong><a href="<?php echo $data['previous']['url'];?>" title="<?php echo $data['previous']['title'];?>"><?php echo $data['previous']['title'];?></a></p>
               <p style="float:right"><strong>下一篇：</strong><a href="<?php echo $data['next']['url'];?>" title="<?php echo $data['next']['title'];?>"><?php echo $data['next']['title'];?></a></p>
            </p><?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
          ...
          <div class="bk15"></div>
            <?php if(module_exists('mood')) { ?><script type="text/javascript" src="<?php echo APP_PATH;?>index.php?m=mood&c=index&a=init&id=<?php echo id_encode($catid,$id);?>"></script><?php } ?>
      </div>
	  ...
	  <span id='favorite'>
		<a href="javascript:;" onclick="add_favorite('<?php echo $title;?>');" class="t6">收藏</a>
	  </span>
    ...
    <div class="col-auto">
        <div class="box">
			...
			<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=119a6a27802fbe8afb122bcfc2838623&action=lists&catid=%24catid&num=10&order=views+DESC&cache=3600\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$tag_cache_name = md5(implode('&',array('catid'=>$catid,'order'=>'views DESC',)).'119a6a27802fbe8afb122bcfc2838623');if(!$data = tpl_cache($tag_cache_name,3600)){$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$data = $content_tag->lists(array('catid'=>$catid,'order'=>'views DESC','limit'=>'10',));}if(!empty($data)){setcache($tag_cache_name, $data, 'tpl_data','','template',3600);}}?>
			<ul class="content digg">
				<?php $n=1;if(is_array($data)) foreach($data AS $v) { ?>
					<li><a href="<?php echo $v['url'];?>" target="_blank" title="<?php echo $v['title'];?>" <?php echo title_style($v[style]);?>><?php echo str_cut($v['title'],28);?></a></li>
				<?php $n++;}unset($n); ?>
				</ul>
			<?php if(defined('IN_ADMIN') && !defined('HTML')) {echo '</div>';}?>
        </div>
