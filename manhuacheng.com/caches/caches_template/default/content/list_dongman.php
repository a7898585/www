...
<?php $footer_txt = "漫画城".$catname."动漫频道为您提供".$catname."动漫，".$catname."动漫观看，最新".$catname."动漫,热门".$catname."动漫，漫画城是最齐全的动漫大全、国内最大的动漫城。"?>
<?php include template("content","dm_nav"); ?>
	<div class="left c666">您现在的位置：&nbsp;<a class="LinkPath" href="/">首页</a>&nbsp;&gt;&nbsp;<a class="LinkPath" href="/dongman">动漫城</a>&nbsp;&gt;&nbsp;<?php echo $catname;?>动漫</div>
    <div class="middle_div1">
    	<div class="middle_div1_le" style="border:1px solid #AEDD5E">
        	<div class="jingdian_tit">
            	<i class="icon"></i>
            	<h3><?php echo $catname;?>动漫列表</h3>
            </div>
            <div class="jingdian_txt">
            	<ul>
			 <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=120a3bacd7180c4b2964be9116fc422c&action=lists&catid=%24catid&order=updatetime+DESC&num=16&page=%24page\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$pagesize = 16;$page = intval($page) ? intval($page) : 1;if($page<=0){$page=1;}$offset = ($page - 1) * $pagesize;$content_total = $content_tag->count(array('catid'=>$catid,'order'=>'updatetime DESC','limit'=>$offset.",".$pagesize,'action'=>'lists',));$pages = pages($content_total, $page, $pagesize, '');$data = $content_tag->lists(array('catid'=>$catid,'order'=>'updatetime DESC','limit'=>$offset.",".$pagesize,'action'=>'lists',));}?>	
