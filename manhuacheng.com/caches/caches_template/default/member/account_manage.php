	...
	<div class="col-1 ">
		...
		<div class="content">
			<div class="col-left himg">
				<a title="<?php echo L('modify').L('avatar');?>" href="index.php?m=member&c=index&a=account_manage_avatar&t=1"><img src="<?php echo $avatar;?>" width="60" height="60" onerror="this.src='<?php echo IMG_PATH;?>nophoto.gif'"></a>
			</div>
			<div class="col-auto">
				<h5><?php if($memberinfo['vip']) { ?><img src="<?php echo IMG_PATH;?>icon/vip.gif"><?php } elseif ($memberinfo['overduedate']) { ?><img src="<?php echo IMG_PATH;?>icon/vip-expired.gif" title="<?php echo L('overdue');?>，<?php echo L('overduedate');?>：<?php echo format::date($memberinfo['overduedate'],1);?>"><?php } ?><?php if($memberinfo['nickname']) { ?> <?php echo $memberinfo['nickname'];?> <?php } else { ?> <?php echo $memberinfo['username'];?><?php } ?> <?php if($memberinfo['email']) { ?>（<?php echo $memberinfo['email'];?>）<?php } ?></h5>
				<p class="blue">
