...
function change_images(uploadid,returnid){
	var d = window.top.art.dialog({id:uploadid}).data.iframe;
	var in_content = d.$("#att-status").html().substring(1);
	var in_more = d.$("#att-more").html();
	var more = in_more.split('|');
	...
	$.each( contents, function(i, n) {
		...
		str += "<li id='image"+ids+"'><input type='text' name='"+returnid+"_url[]' value='"+n+"' style='width:310px;' ondblclick='image_priview(this.value);' class='input-text'> <input type='text' name='"+returnid+"_alt[]' value='图片说明"+(i+1)+"' style='width:160px;' class='input-text' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\">";
		for(var k = 0; k <more.length; k++){
		str += " <input type='text' name='"+returnid+"_"+more[k]+"[]' value='"+more[k]+(i+1)+"' style='width:160px;' class='input-text' onfocus=\"if(this.value == this.defaultValue) this.value = ''\" onblur=\"if(this.value.replace(' ','') == '') this.value = this.defaultValue;\">";
		}
		str += "<a href=\"javascript:remove_div('image"+ids+"')\">移除</a> </li>";
		});
