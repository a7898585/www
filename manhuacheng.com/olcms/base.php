<?php
/**
 * base.php OLCMS框架入口文件
 *
 * @copyright			(C) 2005-2010 OLCMS
 * @license				http://www.olcms.cn/license/
 * @lastmodify			2010-6-7
 */
define ( 'IN_OLCMS', true );

//OLCMS框架路径
define ( 'PC_PATH', dirname ( __FILE__ ) . DIRECTORY_SEPARATOR );

if (! defined ( 'OLCMS_PATH' ))
	define ( 'OLCMS_PATH', PC_PATH . '..' . DIRECTORY_SEPARATOR );
//缓存文件夹地址
define ( 'CACHE_PATH', OLCMS_PATH . 'caches' . DIRECTORY_SEPARATOR );
//主机协议
...
//不允许显示的漫画分类id  add by SouL_Z 2013-4-2 15:03
define ( 'NOT_ALLOW', pc_base::load_config ( 'system', 'not_allow' ) );

//新增变量定义为常量
$m = pc_base::load_config ( 'system', 'setting_add' );
$n = unserialize ( $m );
if (is_array ( $n )) {
	foreach ( $n as $v ) {
		$n = strtoupper ( $v ['varname'] );
		define ( $n, $v ['value'] );
	}
}
