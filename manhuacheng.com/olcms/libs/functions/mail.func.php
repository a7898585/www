...
function sendmail($toemail, $subject, $message, $from='',$cfg = array(), $sitename='') {
	...
	$adminemail = 'olcms.cn@foxmail.com';
	if($cfg && is_array($cfg)) {
		$adminemail = $cfg['from'];
		$mail = $cfg;
	...
	$headers = "From: $email_from{$maildelimiter}X-Priority: 3{$maildelimiter}X-Mailer: OLCMS {$maildelimiter}MIME-Version: 1.0{$maildelimiter}Content-type: text/html; charset=".CHARSET."{$maildelimiter}Content-Transfer-Encoding: base64{$maildelimiter}";
