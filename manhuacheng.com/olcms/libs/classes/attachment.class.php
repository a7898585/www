...
class attachment {
	...
	function __construct($module = '', $catid = 0) {
	...
	function download($field, $value, $watermark = '0', $ext = 'gif|jpg|jpeg|bmp|png', $absurl = '', $basehref = '') {
		...
		if (! preg_match_all ( "/(href|src)=([\"|']?)([^ \"'>]+\.($ext))\\2/i", $string, $matches ) && ! is_array ( $string ))
			return $value;
		if (is_array ( $string )) {
			foreach ( $string as $v ) {
				$remotefileurls [$v] = $v;
		...
		foreach($remotefileurls as $k=>$file) {
			...
			$upload_func = $this->upload_func;
			$gather_img = pc_base::load_config('system','gather_img');
			if ($gather_img) {
				$downrobot = pc_base::load_sys_class ( 'downrobot', '', 0 );
				$PHPURL = new ExpressRobot ( array ('timeout' => 5, 'engine' => 'curl' ) );
				$atturl = parse_url ( $file );
				$host = 'http://' . $atturl ['host'] . '/';
				$r = $PHPURL->getfile ( $file, $newfile, $host ) ? true : false;
			} else {
				$r = $upload_func ( $file, $newfile );
			}
			if ($r) {
				$oldpath [] = $k;
	...
	*/
	...
	function uploaderror($id = 0)	{
		file_put_contents(OLCMS_PATH.'xxx.txt', $id);
	}
	
	/**
	 * 获取站点配置信息
	 */
	private function _get_site_setting() {
