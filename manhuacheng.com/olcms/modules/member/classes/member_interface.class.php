...
class member_interface {
	 ...
	 * @param int $ucuid	ucuid
	 * @param string $username 好友用户名
	 * @param bool {1:成功}
	 */
	public function add_friend($friendid, $userid, $ucuid, $username) {
		$this->favorite_db = pc_base::load_model('friend_model');
		$this->friend_db->insert(array('userid'=>$userid,'ucuid'=>$ucuid, 'friendid'=>$friendid, 'username'=>$username), 1);
		return 1;
	}
