...
<form name="myform" action="?m=member&c=member_setting&a=manage" method="post" id="myform">
	<table width="100%" class="table_form">
		...
		<tr>
			...
			<td>
				<?php echo L('yes')?><input type="radio" name="info[choosemodel]" onclick="showsmtp(this,'mebfg')"  class="input-radio"<?php if($member_setting['choosemodel']) {?>checked<?php }?> value='1'>
				<?php echo L('no')?><input type="radio" name="info[choosemodel]" onclick="showsmtp(this,'mebfg')"  class="input-radio"<?php if(!$member_setting['choosemodel']) {?>checked<?php }?> value='0'>
				<span id="mebfg" style="<?php if($member_setting['choosemodel']) echo 'display:none'?>"><?php echo L('default_modelid')?>：&nbsp;&nbsp;
    			<?php echo form::select($modellist, $member_setting['default_modelid'], 'name="info[default_modelid]"', "选择会员模型");?>
				</span> 
			</td>
		</tr>
		<tr>
