...
class member_verify extends admin {
	...
	function pass() {
		if (isset($_POST['userid'])) {
			...
			foreach($userarr as $v) {
				if($this->ucopen){
					$status = uc_user_register($v['username'], $v['password'], $v['email'],'','', $v['regip']);
					$info['ucuid'] = $status;
				}
	...
	public function checkemail_ajax() {
		...
		if(isset($_GET['ucuid']) && isset($status['uid'])) {
			if ($status['uid'] == intval($_GET['ucuid'])) {
				exit('1');
			}
		}
