...
class content extends foreground {
	...
	public function publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			setcache('member_'.$memberinfo['userid'], $infos,'content');
			//缓存结果 END
			if($info['status']==99) {
			...
			foreach ($CATEGORYS as $catid=>$cat) {
				if($cat['child']==0 && $cat['type']==0) break;
			}
	...
	public function published() {
		...
		$this->content_check_db = pc_base::load_model('content_check_model');
		if($catid){
		$infos = $this->content_check_db->listinfo(array('username'=>$_username,'catid'=>$catid),'inputtime DESC',$page);
		}elseif($modelid){
			$categorys = getcache('category_content','commons');
			...
			$catids = to_sqls($m,'',"catid");
			$sql = "username='$_username' and $catids";

			$infos = $this->content_check_db->listinfo($sql,'inputtime DESC',$page);
		}else{
