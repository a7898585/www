...
class sitemodel_field extends admin {
	...
	public function edit() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			//附加属性值
			$_POST['setting']['more'] = array_filter($_POST['setting']['more']);
			$_POST['info']['setting'] = array2string($_POST['setting']);
	...
	public function delete() {
		$fieldid = intval($_GET['fieldid']);
		$r = $this->db->get_one(array('fieldid'=>$_GET['fieldid']));
		//必须放在删除字段前、在删除字段部分，重置了 tablename
		$this->db->delete(array('fieldid'=>$_GET['fieldid']));
	...
	public function public_field_setting() {
		...
		$modelid = intval($_GET['modelid']);
		pc_base::load_sys_class('form','',0);
		$f_datas = $this->db->select(array('modelid'=>$modelid,'disabled'=>0),'field,name',100,'listorder ASC');
		foreach($f_datas as $_k=>$_v) {
			$exists_field[$_v['field']] = $_v['name'];
		}
		unset($f_datas);
	 ...
	 * 通过模块ID取得字段
	 */
	public function public_field_list() {
		$modelid = intval($_GET['modelid']);
		if($modelid<1) return false;
		$datas = $this->db->select(array('modelid'=>$modelid,'issystem'=>1),'field,name',100,'listorder ASC');
