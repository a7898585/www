...
$content_manage = pc_base::load_config('system', 'content_manage');
?>
...
<!--
	...
	<?php if($content_manage==1){ ?>
	parent.document.getElementById('center_frame').src = '?m=content&c=content&a=public_module&type=add&menuid=<?php echo $_GET['menuid'];?>';
	<?php }elseif($content_manage==2){ ?>
...
<a class="add fb" href="javascript:;" onclick=javascript:openwinx('?m=content&c=content&a=add&menuid=&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>','')><em><?php echo L('add_content');?></em></a>　
<a href="?m=content&c=content&a=init&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>" <?php if($steps==0 && !isset($_GET['reject'])) echo 'class=on';?>><em><?php echo L('check_passed');?></em></a><span>|</span>
...
<option value="0">选择栏目分类</option><?php echo $string;?></select>
...
<div class="table-list">
    <table width="100%">
        <thead>
            ...
            <th width="125"><?php echo L('updatetime');?></th>
...
<tbody>
	...
	?>
		...
		<?php
                if($modelid == 1){
                    echo '<a href="/index.php?m=content&c=index&a=show&catid=19&id='.$r['id'].'" target="_blank">';
                }else if($modelid == 17 ){
                    echo '<a href="/index.php?m=content&c=index&a=show&catid=32&id='.$r['id'].'" target="_blank">';
                }else if($modelid == 18){
                    echo "";
                }
                else if($status==99 && empty($temp)) {
                    echo '<a href="/index.php?m=content&c=index&a=show&catid=14&id='.$r['id'].'" target="_blank">';
                } else if($status==99 &&  $temp){
                    //ydl 2016年3月1日 11:24:29
                    echo '<a href="/manhua'.$r['id'].'" target="_blank">';
                } else {
//                                                            http://www.manhuacheng.com/index.php?m=content&c=index&a=show&catid=14&id=256860
			echo '<a href="javascript:;" onclick=\'window.open("?m=content&c=index&a=show&catid='.$catid.'&id='.$r['id'].'","manage")\'>';
		...
		<td align='center'><a href="javascript:;" onclick="javascript:openwinx('?m=content&c=content&a=edit&catid=<?php echo $r['catid'];?>&id=<?php echo $r['id']?>','')"><?php echo L('edit');?></a> | <a href="javascript:view_comment('<?php echo id_encode('content_'.$r['catid'],$r['id']);?>','<?php echo safe_replace($r['title']);?>')"><?php echo L('comment');?></a></td>
	</tr>
     <?php }
	}
	?>
    ...
    <div class="btn"><label for="check_box"><?php echo L('selected_all');?>/<?php echo L('cancel');?></label>
    	<input type="button" class="button" value="<?php echo L('listorder');?>" onclick="myform.action='?m=content&c=content&a=listorder&dosubmit=1&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';myform.submit();"/>
		...
		<?php }
		if($status!=99) {?>
		<input type="button" class="button" value="<?php echo L('passed_checked');?>" onclick="myform.action='?m=content&c=content&a=pass&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';myform.submit();"/>
		<?php }?>
		<input type="button" class="button" value="<?php echo L('delete');?>" onclick="myform.action='?m=content&c=content&a=delete&dosubmit=1&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>&steps=<?php echo $steps;?>';return confirm_delete()"/>
		<?php if(!isset($_GET['reject'])) { ?>
		<input type="button" class="button" value="<?php echo L('push');?>" onclick="push();" <?php if(!$catid) echo 'disabled';?>/>
		...
		<?php }}?>
		<input type="button" class="button" value="<?php echo L('remove');?>" onclick="myform.action='?m=content&c=content&a=remove&modelid=<?php echo $modelid;?>&catid=<?php echo $catid;?>';myform.submit();"/>
	</div>
