...
<td align="center"><a href="?m=content&c=type_manage&a=init&parentid=<?php echo $r['typeid']?>">下级类别</a> |<a href="javascript:edit('<?php echo $r['typeid']?>','<?php echo trim(new_addslashes($r['name']))?>')"><?php echo L('edit');?></a> | <a href="javascript:;" onclick="data_delete(this,'<?php echo $r['typeid']?>','<?php echo trim(new_addslashes($r['name']));?>')"><?php echo L('delete')?></a> </td>
</tr>
<?php } ?>
	</tbody>
    </table>
...
function data_delete(obj,id,name){
	...
	function(){
	$.get('?m=content&c=type_manage&a=delete&typeid='+id,function(data){
				if(data) {
