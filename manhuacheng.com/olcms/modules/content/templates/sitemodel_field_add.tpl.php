...
<?php echo form::select($all_field,'','name="info[formtype]" id="formtype" onchange="javascript:field_setting(this.value);"',L('select_fieldtype'));?>
	...
	<tr> 
      ...
      <input type="radio" name="info[issystem]" id="field_basic_table1" value="1" onclick="$('#issystem').val('1');"> <?php echo L('yes');?><?php if($model_cache[$modelid]['type']!=3){?> <input type="radio" id="field_basic_table0" name="info[issystem]" value="0" onclick="$('#issystem').val('0');" checked> <?php echo L('no');?><?php }?></td>
    </tr>
	<tr> 
...
<option value="/^[0-9]{6}$/"><?php echo L('zip');?></option>
	...
	<tr> 
      ...
      <td><input type="radio" name="info[isbase]" value="1"  checked> <?php echo L('yes');?> <input type="radio" name="info[isbase]" value="0"> <?php echo L('no');?><input type="radio" name="info[isbase]" value="2"> <?php echo L('no_display');?> </td>
    </tr>
	<tr> 
	...
	<tr> 
      ...
      <td><input type="radio" name="info[isposition]" value="1" /> <?php echo L('yes');?> <input type="radio" name="info[isposition]" value="0" checked/> <?php echo L('no');?></td>
    	...
    	<tr> 
      <th><strong><?php echo L('as_seo_info');?></strong></th>
      <td><input type="radio" name="info[isseo]" value="1"  /> <?php echo L('yes');?> <input type="radio" name="info[isseo]" value="0"  checked/> <?php echo L('no');?></td>
    </tr>
		<tr> 
...
<script type="text/javascript">
	...
	function field_list(modelid) {
		$.getJSON("?m=content&c=sitemodel_field&a=public_field_list&modelid="+modelid, function(data){
			$('#rl_fieldid').html('');
			$('#rl_field').html('');
			$('#rl_fieldid').append('<option value="">选择关联字段对应值</option>');
			$('#rl_field').append('<option value="">选择用来关联字段</option>');
			$('#rl_fieldid').append('<option value="id">ID值</option>');
			for(var i=0; i < data.length; i++){
				$('#rl_fieldid').append('<option value="'+data[i].field+'">'+data[i].name+'</option>');
				$('#rl_field').append('<option value="'+data[i].field+'">'+data[i].name+'</option>');
			}
			$('#rl_field').children().eq(0).attr({selected:true});
			$('#rl_fieldid').children().eq(0).attr({selected:true});		
		});
		str='<?php foreach ($f_datas as $v){?><option value="<?php echo $v['field'];?>"><?php echo $v['name'];?></option><?php }?>';
		$('#rl_field_value').append(str);
	}
