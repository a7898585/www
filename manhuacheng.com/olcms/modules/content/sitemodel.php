...
class sitemodel extends admin {
	...
	public function init() {
		...
		$datas = $this->db->listinfo('type=0 or type=3','',$_GET['page']);
		//模型文章数array('模型id'=>数量);
		$items = array();
		foreach ($datas as $k=>$r) {
			...
			$datas[$k]['items'] = $items[$r['modelid']] ? $items[$r['modelid']] : 0;
		}
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			if($_POST['info']['type']==3)
			$model_sql = file_get_contents(MODEL_PATH.'one_model.sql');
			else
			...
			$this->type_db->insert(array('name'=>$_POST['info']['name'],'module'=>'search','modelid'=>$modelid));
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			$this->db->update($_POST['info'],array('modelid'=>$modelid));
	...
	public function delete() {
		...
		$type = $model_cache[$modelid]['type'];
		$this->sitemodel_field_db->delete(array('modelid'=>$modelid));
		$this->db->drop_table($model_table);
		if($type!=3)
		$this->db->drop_table($model_table.'_data');
		
		$this->db->delete(array('modelid'=>$modelid));
		//删除全站搜索接口数据
		$this->type_db = pc_base::load_model('type_model');
		$this->type_db->delete(array('module'=>'search','modelid'=>$modelid));
		$cache_api = pc_base::load_app_class('cache_api','admin');
		$cache_api->cache('type');
		$cache_api->search_type();
		exit('1');
	...
	public function disabled() {
		...
		$this->db->update(array('disabled'=>$status),array('modelid'=>$modelid));
	...
	public function public_cache() {
		...
		//更新模型数据缓存
		$model_array = array();
		$datas = $this->db->select('type=0 or type=3');
		foreach ($datas as $r) {
		...
		if(isset($_POST['dosubmit'])) {
			...
			if($modelid){
				...
				if(!empty($model_import_data)) {
					$this->sitemodel_field_db = pc_base::load_model('sitemodel_field_model');
					$system_field = array('title','style','catid','url','listorder','status','userid','username','inputtime','updatetime','pages','readpoint','template','groupids_view','posids','content','keywords','description','thumb','typeid','relation','islink','allow_comment','views');
	...
	public function cache_field($modelid = 0) {
		...
		$field_array = array();
		$fields = $this->field_db->select(array('modelid'=>$modelid),'*',100,'listorder ASC');
