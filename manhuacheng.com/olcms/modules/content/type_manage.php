...
class type_manage extends admin {
	...
	public function init () {
		...
		$result_datas = $this->db->listinfo(array('module'=>'content','parentid'=>$parentid),'listorder ASC',$_GET['page']);
		$pages = $this->db->pages;
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
			...
			foreach ($names as $name) {
				...
				if(!empty($ids)) {
					foreach ($ids as $catid) {
						...
						$this->category_db->update(array('usable_type'=>$usable_type),array('catid'=>$catid));
					}
				}
			}
			...
			$categorys = $this->public_getsite_categorys();
 			$typedatas = $this->db->select(array('module'=>'content','parentid'=>0));
 			foreach($typedatas as $v){
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			if($catids_string) {	
				...
				$diff_ids = array_diff($catids_string, $ids);
				foreach ($diff_ids as $catid){
					$r = $this->category_db->get_one(array('catid'=>$catid),'usable_type');
					if($r['usable_type']) {
						$usable_type = array();
						$usable_type_arr = explode(',', $r['usable_type']);
						$key = array_search($typeid, $usable_type_arr);
						unset($usable_type_arr[$key]);
						$usable_type_arr = array_filter($usable_type_arr);
	...
	public function cache() {
		$datas = array();
		$result_datas = $this->db->select(array('module'=>'content'),'*',1000,'listorder ASC');
	...
	private function category_cache() {
		$categorys = array();
		$this->categorys = $this->category_db->select("",'*',10000,'listorder ASC');
		foreach($this->categorys as $r) {
			unset($r['module']);
