...
class html {
	...
	public function show($file, $data = '', $array_merge = 1,$action = 'add',$upgrade = 0) {
		...
		//上一页
		$previous_page = $this->db->get_one("`catid` = '$catid' AND `id`<'$id' AND `status`=99",'title,url','id DESC');
		//下一页
		$next_page = $this->db->get_one("`catid`= '$catid' AND `id`>'$id' AND `status`=99",'title,url');
	...
	public function category($catid, $page = 0) {
		...
		$base_file = '/'.$base_file;
		//检查是否生成到根目录
		$create_to_html_root = $CAT['create_to_html_root'];
		
		//判断二级域名是否直接绑定到该栏目
		$root_domain = preg_match('/^http:\/\/([a-z0-9\-\.]+)\/$/',$CAT['url']) ? 1 : 0;

		//检查域名所绑定的原始目录，看是否是设置了生成到根目录
		if(preg_match("/^(http:\/\/)?([^\/]+)/i", $CAT['url'],$_match)) {
			if($CAT['parentid'] && $_match[0]) {
		...
		if($create_to_html_root) {
			$file = OLCMS_PATH.$base_file;

			//添加到发布点队列
			$this->queue->add_queue('add',$base_file);
			//评论跨站调用所需的JS文件
			...
			$file = OLCMS_PATH.substr($this->html_root,1).$base_file;
			//添加到发布点队列
	...
	public function index() {
		$file = OLCMS_PATH.'index.html';
		//添加到发布点队列
		$this->queue->add_queue('edit','/index.html');
		//SEO
		$SEO = seo();
		$CATEGORYS = $this->categorys;
		ob_start();
	...
	*/
	private function createhtml($file, $copyjs = '') {
		...
		@chmod($file,0777);
		if(!is_writable($file)) {
			$file = str_replace(OLCMS_PATH,'',$file);
