...
class block_admin extends admin {
	...
	public function init() {
		...
		if ($_SESSION['roleid'] != 1) {
			$offset = ($page-1) * 20;
			$r = $this->priv_db->select(array('roleid'=>$this->roleid),'blockid', $offset.',20');
			$blockid_list = array();
			foreach ($r as $key=>$v) {
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit'])) {
			...
			if ($this->db->update(array('name'=>$name), array('id'=>$id))) {
				//设置权限
				$priv = isset($_POST['priv']) ? $_POST['priv'] : '';
				$this->priv_db->delete(array('blockid'=>$id));
				if (!empty($priv)) {
					if (is_array($priv)) foreach ($priv as $v) {
						if (empty($v)) continue;
						$this->priv_db->insert(array('roleid'=>$v, 'blockid'=>$id));
		...
		$administrator = getcache('role', 'commons');
		unset($administrator[1]);
		$r = $this->priv_db->select(array('blockid'=>$id),'roleid');
		$priv_list = array();
		foreach ($r as $v) {
	...
	public function block_update() {
		...
		//进行权限判断
		if ($this->roleid != 1) {
			if (!$this->priv_db->get_one(array('blockid'=>$id, 'roleid'=>$this->roleid))) {
