...
class attachments extends admin  {
	...
	public function crop_upload() {	
		if (isset($GLOBALS["HTTP_RAW_POST_DATA"])) {
			...
			if (isset($_GET['file']) && !empty($_GET['file'])) {
				...
				file_put_contents(OLCMS_PATH.$filepath.$new_file, $pic);
			} else {
				return false;
			}
	...
	public function album_dir() {
		...
		$local = str_replace(array(PC_PATH, OLCMS_PATH ,DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('','',DIRECTORY_SEPARATOR), $filepath);
