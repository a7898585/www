...
class index {
	protected  $setting, $catid, $contentid, $mood_id, $modelid;
	public function __construct() {
		...
		list($this->modelid, $this->catid, $this->contentid) = id_decode($this->mood_id);
	}
	...
	public function init() {
		...
		$data = $mood_db->get_one(array('catid'=>$this->catid, 'contentid'=>$this->contentid));
	...
	public function post() {
		...
		if (in_array($this->mood_id, $cookie)) {
			...
			$fields = 'n'.$key;
			if ($data = $mood_db->get_one(array('catid'=>$this->catid, 'contentid'=>$this->contentid))) {
				$mood_db->update(array('total'=>'+=1', $fields=>'+=1', 'lastupdate'=>SYS_TIME), array('id'=>$data['id']));
				$data['total']++;
				$data[$fields]++;
			} else {
				$mood_db->insert(array('total'=>'1', $fields=>'1','modelid'=>$this->modelid, 'catid'=>$this->catid, 'contentid'=>$this->contentid,'
				lastupdate'=>SYS_TIME));
				$data['total'] = 1;
				$data[$fields] = 1;
			}
