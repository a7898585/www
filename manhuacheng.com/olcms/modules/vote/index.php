...
class index {
	...
	public function post(){
		...
		$return = $this->check($subjectid);
 		switch ($return) {
		case 0:
		  showmessage(L('vote_voteyes'),"?m=vote&c=index&a=result&subjectid=$subjectid");
		  break;
		case -1:
		  showmessage(L('vote_voteyes'),"?m=vote&c=index&a=result&subjectid=$subjectid");
		  break;
		...
		showmessage(L('vote_votesucceed'), "?m=vote&c=index&a=result&subjectid=$subjectid");
	}
	...
	public function check($subjectid){
		...
		if($subject_arr['enabled']==0){
			showmessage(L('vote_votelocked'),"?m=vote&c=index&a=result&subjectid=$subjectid");
 		}
 		if(date("Y-m-d",SYS_TIME)>$subject_arr['todate']){
			showmessage(L('vote_votepassed'),"?m=vote&c=index&a=result&subjectid=$subjectid");
 		}
 		//游客是否可以投票
		if($subject_arr['allowguest']==0&&!$this->username){
			showmessage(L('vote_votenoguest'),"?m=vote&c=index&a=result&subjectid=$subjectid");
		}
 		//是否有投票记录 
