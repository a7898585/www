...
class node extends admin {
	...
	public function manage() {
		...
		$nodelist = $this->db->listinfo('', 'nodeid DESC', $page, 15);
		$pages = $this->db->pages;
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
			...
			if ($total_page > 0) {
				...
				if (is_array($url) && !empty($url)) foreach ($url as $v) {
					...
					$md5 = md5($v['url']);
					if (!$history_db->get_one(array('md5'=>$md5))) {
	...
	public function col_content() {
		...
		if ($data = $this->db->get_one(array('nodeid'=>$nodeid))) {
			...
			if (empty($total)) $total = $content_db->count(array('nodeid'=>$nodeid, 'status'=>0));
			$total_page = ceil($total/2);
			$list = $content_db->select(array('nodeid'=>$nodeid, 'status'=>0), 'id,url', '2', 'id desc');
			$i = 0;
			...
			if ($total_page > $page) {
				showmessage(L('collectioning').($i+($page-1)*2).'/'.$total.'<script type="text/javascript">location.href="?m=collection&c=node&a=col_content&page='.($page+1).'&nodeid='.$nodeid.'&total='.$total.'"</script>', '?m=collection&c=node&a=col_content&page='.($page+1).'&nodeid='.$nodeid.'&total='.$total);
			} else {
				$this->db->update(array('lastdate'=>SYS_TIME), array('nodeid'=>$nodeid));
	...
	public function import() {
		...
		$program_list = $program_db->select(array('nodeid'=>$nodeid), 'id, catid');
	...
	public function import_program_add() {
		...
		//读取栏目缓存
		$catlist = getcache('category_content', 'commons');
		$cat = $catlist[$catid];
	...
	public function import_content() {
		...
		$url = '?m=collection&c=node&a=publist&nodeid='.$nodeid.'&status=2';
		if ($type == 'all') {
			$total = isset($_GET['total']) && intval($_GET['total']) ? intval($_GET['total']) : '';
			if (empty($total)) $total = $collection_content_db->count(array('nodeid'=>$nodeid, 'status'=>1));
			$total_page = ceil($total/20);
			$page = isset($_GET['page']) && intval($_GET['page']) ? intval($_GET['page']) : 1;
			$total_page = ceil($total/20);
			$data = $collection_content_db->select(array('nodeid'=>$nodeid, 'status'=>1), 'id, data', '20', $order);
			
		} else {
			$ids = explode(',', $ids);
			$ids = implode('\',\'', $ids);
			$data = $collection_content_db->select("id in ('$ids') AND nodeid = '$nodeid' AND status = '1'", 'id, data', '', $order);
			$total = count($data);
		...
		if ($type == 'all' && $total_page > $page) {
			$str = L('are_imported_the_import_process').(($page-1)*20+$i).'/'.$total.'<script type="text/javascript">location.href="?m=collection&c=node&a=import_content&nodeid='.$nodeid.'&programid='.$programid.'&type=all&page='.($page+1).'&total='.$total.'"</script>';
			$url = '';
		}
		showmessage($str, $url);
	}
