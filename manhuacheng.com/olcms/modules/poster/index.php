...
class index {
	...
	public function poster_click() {
		...
		if($id) {
			$this->s_db->insert(array('pid'=>$id, 'username'=>$username, 'area'=>$area, 'ip'=>$ip, 'referer'=>HTTP_REFERER, 'clicktime'=>SYS_TIME, 'type'=> 1));
		}
	 ...
	 * @return boolen 
	 */
	private function show_stat($spaceid = 0, $id = 0) {
		...
		$this->s_db->insert(array('pid'=>$id, 'spaceid'=>$spaceid, 'username'=>$username, 'area'=>$area, 'ip'=>$ip, 'referer'=>HTTP_REFERER, 'clicktime'=>SYS_TIME, 'type'=>0));
		return true;
	}
}
?>