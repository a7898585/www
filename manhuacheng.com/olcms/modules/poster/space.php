...
class space extends admin {
	...
	public function init() {
		...
		$infos = $this->db->listinfo('', '`spaceid`', $page);
		$pages = $this->db->pages;
	...
	public function add() {
		if (isset($_POST['dosubmit'])) {
			...
			if ($spaceid) {
				if ($space['type']=='code') {
					$path = '{show_ad('.$spaceid.')}';
				} else {
					$path = 'poster_js/'.$spaceid.'.js';
				}
				$this->db->update(array('path'=>$path), array('spaceid'=>$spaceid));
	...
	public function public_call() {
		...
		$r = $this->db->get_one(array('spaceid'=>$_GET['sid']));
	...
	public function public_preview() {
		if (is_numeric($_GET['spaceid'])) {
			$_GET['spaceid'] = intval($_GET['spaceid']);
			$r = $this->db->get_one(array('spaceid'=>$_GET['spaceid']));
			$scheme = $_SERVER['SERVER_PORT'] == '443' ? 'https://' : 'http://';
			if ($r['type']=='code') {
				$db = pc_base::load_model('poster_model');
				$rs = $db->get_one(array('spaceid'=>$r['spaceid']), 'setting', '`id` ASC');
				if ($rs['setting']) {
	...
	private function _del($spaceid = 0) {
		...
		$db = pc_base::load_model('poster_model');
		$db->delete(array('spaceid'=>$spaceid));
		$this->db->delete(array('spaceid' => $spaceid));
		return true;
	}
	...
	public function create_js($page = 0) {
		$page = max(intval($_GET['page']), 1);
		if ($page==1) {
			$result = $this->db->get_one(array('disabled'=>0), 'COUNT(*) AS num');
			if ($result['num']) {
				$total = $result['num'];
				$pages = ceil($total/20);
			}
		...
		$offset = ($page-1)*20;
		$data = $this->db->listinfo(array('disabled'=>0), 'spaceid ASC', $page);
	...
	public function public_check_space() {
		...
		if ($_GET['spaceid']) {
			...
			$r = $this->db->get_one(array('spaceid' => $spaceid));
			if ($r['name'] == $name) {
				exit('1');
			}
		} 
		$r = $this->db->get_one(array('name' => $name), 'spaceid');
		if ($r['spaceid']) {
			exit('0');
		} else {
			exit('1');
	...
	private function check($data = array()) {
		...
		$info = $this->db->get_one(array('name'=>$data['name']), 'spaceid');
