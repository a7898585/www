...
class index {
	protected $commentid, $modules, $format;
	function __construct() {
		...
		list($this->modules, $contenti) = decode_commentid($this->commentid);
	...
	public function init() {
		...
		$username = param::get_cookie('_username',L('OLcms_friends'));
		$userid = param::get_cookie('_userid');
		
		$comment_setting_db = pc_base::load_model('comment_setting_model');
		$setting = $comment_setting_db->get_one ( array ('id' => 1 ) );
		//SEO
		$SEO = seo ();
	...
	public function post() {
		...
		$username = param::get_cookie ( '_username', $SITE ['name'] . L ( 'olcms_friends' ) );
		$userid = param::get_cookie ( '_userid' );
