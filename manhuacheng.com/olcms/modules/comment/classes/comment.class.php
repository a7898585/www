...
class comment {
	...
	public function add($commentid, $data, $id = '', $title = '', $url = '') {
		...
		if (!$comment = $this->comment_db->get_one(array('commentid'=>$commentid), 'tableid, commentid')) { //评论不存在
			//取得当前可以使用的内容数据表
			...
			//新建评论到评论总表中。
			$comment_data = array('commentid'=>$commentid, 'tableid'=>$tableid, 'display_type'=>($data['direction']>0 ? 1 : 0));
		...
		//判断当前站点是否需要审核
		$site = $this->comment_setting_db->site();
		if ($site['check']) {
			$data['status'] = 0;
		}
	...
	public function status($commentid, $id, $status = -1) {
		...
		//读取评论的站点配置信息
		$site = $this->comment_setting_db->get_one(array('id'=>1));
