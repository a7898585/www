...
class comment_admin extends admin {
	private $comment_setting_db,$comment_data_db,$comment_db;
	...
	public function init() {
		$data = $this->comment_setting_db->get_one(array('id'=>1));
		if (isset($_POST['dosubmit'])) {
			...
			if ($data) {
				$this->comment_setting_db->update($sql, array('id'=>1));
			} else {
				$sql['id'] = 1;
	...
	public function listinfo() {
		...
		if (isset($_GET['search'])) {
			...
			switch ($searchtype) {
				case '0':
					$sql = "SELECT `commentid` FROM `olcms_comment` WHERE  `title` LIKE '%$keywords%' AND `tableid` = '$tableid' ";
				...
				case '1':
					...
					$sql = "SELECT `commentid` FROM `olcms_comment` WHERE `commentid` LIKE 'content_%-$keywords-%' ";
					$this->comment_db->query($sql);
					$data = $this->comment_db->fetch_array();
					foreach ($data as $d) {
						$comment_id .= $t.'\''.$d['commentid'].'\'';
