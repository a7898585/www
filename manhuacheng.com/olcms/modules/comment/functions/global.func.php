...
/**
 * 通过API接口调用标题和URL数据
 * @param string $commentid    评论ID:content_8-351
 * @return array($title, $url)   返回数据
 */
function get_comment_api($commentid) {
	list($modules, $contentid) = id_decode($commentid);
	if (empty($modules) || empty($contentid)) {
		return false;
	}
	$comment_api = '';
	...
	return $comment_api->get_info($modules, $contentid);
}