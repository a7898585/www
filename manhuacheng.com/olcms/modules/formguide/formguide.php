...
class formguide extends admin {
	...
	public function init() {
		...
		$data = $this->db->listinfo(array('type' => 3), '`modelid` DESC', $page);
	...
	public function disabled() {
		...
		$this->db->update(array('disabled'=>$val), array('modelid'=>$formid));
	...
	public function delete() {
		if (isset($_GET['formid']) && !empty($_GET['formid'])) {
			...
			$m_db->delete(array('modelid'=>$formid));
			...
			$this->db->delete(array('modelid'=>$formid));
			showmessage(L('operation_success'), HTTP_REFERER);
			...
			if (is_array($_POST['formid'])) {
				foreach ($_POST['formid'] as $fid) {
					...
					$tablename = $m_db->db_tablepre.'form_'.$m_info['tablename'];
					$m_db->query("DROP TABLE `$tablename`");
					$this->db->delete(array('modelid'=>$fid));
				}
			}
			showmessage(L('operation_success'), HTTP_REFERER);
		} else {
	...
	private function sql_split($sql) {
		...
		$sql = str_replace("olcms_form_table", $this->m_db->db_tablepre.'form_'.$this->tablename, $sql);
		$ret = array();
		$num = 0;
