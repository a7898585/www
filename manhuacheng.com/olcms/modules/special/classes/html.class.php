...
class html {
	...
	public function _create_content($contentid = 0) {
		...
		if ($CONTENT_POS !== false) {
			...
			for ($i=1; $i<=$pagenumber; $i++) {
				...
				$this->queue->add_queue('add', $file);
				$file = OLCMS_PATH.$file; //生成文件的路径
	...
	public function _index($specialid = 0, $pagesize = 20, $pages_num = 0) {
		...
		$r = $this->db->get_one(array('id'=>$specialid));
		...
		if(!$ispage) {
			...
			$types = $type_db->select(array('module'=>'special', 'parentid'=>$specialid), '*', '', '`listorder` ASC, `typeid` ASC', '', 'typeid');
		}
		...
		if($ispage) {
			...
			for ($i=1; $i<=$pages_num; $i++) {
				...
				$this->queue->add_queue('add', $file_root); //添加至信息队列
				$file_root = OLCMS_PATH.$file_root;
				ob_start();
				include template('special', $template);
			...
			$this->queue->add_queue('add', $file); //添加至信息队列
			$file = OLCMS_PATH.$file;
			ob_start();
	 ...
	 */
	public function create_type($specialid = 0, $page = 1) {
		if (!$specialid) return false;
		$infos = $this->type_db->select(array('module'=>'special','parentid'=>$specialid));
		...
		$GLOBALS['URL_ARRAY'] = array('specialdir'=>$filename, 'typedir'=>$info['typedir'], 'typeid'=>$specialid);
		$SEO = seo('', $info['typename'], '');
		$template = $list_template ? $list_template : 'list';
		if ($page==1) $file = pc_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/type-'.$info['typeid'].'.html';
		else $file = pc_base::load_config('system', 'html_root').'/special/'.$filename.'/'.$info['typedir'].'/type-'.$info['typeid'].'-'.$page.'.html';
		$this->queue->add_queue('add', $file);
		$file = OLCMS_PATH.$file;
		$typeid = $info['typeid'];
		ob_start();
