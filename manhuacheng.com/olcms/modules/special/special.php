...
class special extends admin {
	...
	public function init() {
		if($_GET['sortid']){
			$where = array('sortid'=>$_GET['sortid']);
		}else{
			$where = '';
		}
		$page = max(intval($_GET['page']), 1);
		$infos = $this->db->listinfo($where, '`listorder` DESC, `id` DESC', $page, 6);
		$types = $this->db2->listinfo(array('module'=>ROUTE_M,'parentid'=>0),$order = 'typeid DESC');
	...
	public function add() {
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($id) {
				...
				if ($special ['isurl'] || $special ['ishtml']) {
					$url = $special ['ishtml'] ? APP_PATH . pc_base::load_config ( 'system', 'html_root' ) . '/special' . $special ['url'] : APP_PATH . $special ['alias'] . '/';
				} else {
					$url = '/index.php?m=special&c=index&id=' . $id;
				}
			...
			$typedatas = $this->db2->select(array('module'=> ROUTE_M,'parentid'=>0),'*','',$order = 'typeid ASC');
			foreach($typedatas as $k=>$v){
				$typedata[$v['typeid']] = $v['name'];
			}
			unset($typedatas);
	...
	public function edit() {
		...
		if (isset($_POST['dosubmit']) && !empty($_POST['dosubmit'])) {
			...
			if ($special ['isurl'] || ($special['ishtml'] && $special['filename'])) {
				$special['url'] = $special ['ishtml'] ? pc_base::load_config ( 'system', 'html_root' ) . '/special/' . $special ['filename'] . '/' : APP_PATH . $special ['alias'] . '/';
			} else {
				$special['url'] = '/index.php?m=special&c=index&id=' . $_GET['specialid'];
			}
			...
			if ($special['ishtml']) {
				...
				$html->create_type($_GET['specialid']);
			}
			//更新附件状态
			if(pc_base::load_config('system','attachment_stat')) {
				$this->attachment_db = pc_base::load_model('attachment_model');
			...
			$info = $this->db->get_one(array('id'=>$_GET['specialid']));
			//获取站点模板信息
			pc_base::load_app_func('global', 'admin');
			$template_list = template_list(0);
			foreach ($template_list as $k=>$v) {
			...
			$types = $type_db->select(array('module'=>ROUTE_M, 'parentid'=>$_GET['specialid']), '`typeid`, `name`, `typedir`', '', '`listorder` ASC, `typeid` ASC');
	...
	public function html() {
		if((!isset($_POST['id']) || empty($_POST['id']))) {
			$result = $this->db->select(array('disabled'=>0), 'id', '', '', '', 'id');
	...
	public function elite() {
		...
		$this->db->update(array('elite'=>$_GET['value']), array('id'=>$_GET['id']));
		showmessage(L('operation_success'), HTTP_REFERER);
	}
	...
	private function special_cache() {
		$specials = array();
		$result = $this->db->select(array('disabled'=>0), '`id`, `title`, `url`, `thumb`, `banner`, `ishtml`', '', '`listorder` DESC, `id` DESC');
		foreach($result as $r) {
			$specials[$r['id']] = $r;
		}
	...
	public function public_check_special() {
		...
		$r = $this->db->get_one(array('title' => $title), 'id');
		if($r['id']) {
			exit('0');
		} else {
			exit('1');
	...
	public function public_check_dir() {
		...
		$r = $this->db->get_one(array('filename'=>$_GET['filename']), 'id');
		if($r['id']) {
			exit('0');
		} else {
			exit('1');
	...
	private function check($data, $a = 'add') {
		...
		if ($data['voteid']) {
			if (strpos($data['voteid'], '|')===false) {
				...
				$r = $vote_db->get_one(array('subject'=>$data['voteid']), 'subjectid, subject', 'addtime DESC');
				if ($r) {
	...
	public function add_type() {
		if(isset($_POST['dosubmit'])) {
			...
			$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=special&c=special&a=add_type\', title:\''.L('special_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('special'));
 			include $this->admin_tpl('special_type_add');
		}
	...
	public function edit_type() {
		if(isset($_POST['dosubmit'])){ 
			...
			showmessage(L('operation_success'),'/index.php?m=special&c=special&a=list_type','', 'edit');
			...
			extract($info);
			include $this->admin_tpl('special_type_edit');
		}
	...
	public function list_type() {
		...
		$infos = $this->db2->listinfo(array('module'=> ROUTE_M,'parentid'=>0),$order = 'listorder DESC',$page, $pages = '10');
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=special&c=special&a=add\', title:\''.L('special_add').'\', width:\'700\', height:\'450\'}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('special'));
		include $this->admin_tpl('special_list_type');
	}
}
?>