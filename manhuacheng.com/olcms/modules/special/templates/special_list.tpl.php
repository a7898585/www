...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		...
		<?php echo L('all_type')?>: &nbsp;&nbsp; <a href="?m=special&c=special"><?php echo L('all')?></a> &nbsp;&nbsp;
		<?php
	if(is_array($type_arr)){
	foreach($type_arr as $typeid => $type){
		?><a href="?m=special&c=special&sortid=<?php echo $typeid;?>"><?php echo $type;?></a>&nbsp;
		<?php }}?>
		</div>
		</td>
		</tr>
...
<p class="gray4"><?php echo L('create_man')?>：<a href="#" class="blue"><?php echo $info['username']?></a>， <?php echo L('create_time')?>：<?php echo format::date($info['createtime'], 1)?></p>
	...
	<td align="center"><span style="height:22"><a href='?m=special&c=content&a=init&specialid=<?php echo $info['id']?>' onclick="javascript:openwinx('?m=special&c=content&a=add&specialid=<?php echo $info['id']?>','')"><?php echo L('add_news')?></a></span> | 
