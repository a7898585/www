...
<span id="catids"></span>&nbsp;&nbsp; 
				...
				<link rel="stylesheet" type="text/css" href="statics/js/calendar/calendar-blue.css"/>
					...
					date = new Date();document.getElementById ("start_time").value="<?php echo $_GET['start_time'] ? $_GET['start_time'] : format::date(SYS_TIME-4892000);?>";
					Calendar.setup({
...
<script type="text/javascript">
	function select_categorys(modelid, id) {
		if(modelid) {
			$.get('', {m: 'special', c: 'special', a: 'ajax_categorys_list', modelid: modelid, catid: id }, function(data){
				if(data) {
					$('#catids').html(data);
				} else $('#catids').html('');
			});
