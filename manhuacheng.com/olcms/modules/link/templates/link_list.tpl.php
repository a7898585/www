...
<table width="100%" cellspacing="0" class="search-form">
	...
	foreach($type_arr as $typeid => $type){
		...
		<?php }}?>
		</div><div class="explain-col">
		<form name="searchform" action="" method="get" >
<input type="hidden" value="link" name="m">
<input type="hidden" value="link" name="c">
<input type="hidden" value="init" name="a">

<select name="typeid"><option>选择分类</option><?php if(is_array($type_arr)){ foreach($type_arr as $typeid => $type){?><option value='<?php echo $typeid;?>' <?php if($_GET['typeid']==$typeid) echo 'selected';?>><?php echo $type;?></option><?php }?>
<?php }?></select>
		<select name="type"><option>筛选条件</option><option value="1">名称搜索</option><option value="1">网址搜索</option></select>
		<input name="keyword" type="text" value="<?php if(isset($keyword)) echo $keyword;?>" class="input-text" />
		<input type="submit" name="search" class="button" value="<?php echo L('search');?>" /></form>
		</div>
		</td>
		</tr>
    </tbody>
...
?>
	<tr>
		...
		<td align="center" width="35"><a href="<?php echo $info['url'];?>" title="<?php echo L('go_website')?>" target="_blank"><?php echo $info['name']?></a> </td>
