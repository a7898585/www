...
class cache_api {
	 ...
	 * 更新管理员角色缓存方法
	 */
	public function _cache_site($role) {
		$priv_db = pc_base::load_model('admin_role_priv_model');
		foreach($role as $n=>$r) {
			$sitelists = $priv_db->select(array('roleid'=>$n),'roleid', '', 'roleid');
	...
	public function type($param = '') {
		$datas = array();
		$result_datas = $this->db->select(array('module'=>$param),'*',1000,'listorder ASC,typeid ASC');
		foreach($result_datas as $_key=>$_value) {
			$datas[$_value['typeid']] = $_value;
		}
		if ($param=='search') {
			setcache('search_types',$datas, 'search');
			$this->search_type();
		} else {
	...
	public function workflow() {
		$datas = array();
		$workflow_datas = $this->db->select('','*',1000);
		foreach($workflow_datas as $_k=>$_v) {
			$datas[$_v['workflowid']] = $_v;
		}
		setcache('workflow',$datas,'commons');
		return true;
	}
	...
	public function membermodel() {
		...
		//模型缓存路径
		define('MEMBER_CACHE_MODEL_PATH',OLCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_model'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR);
	...
	public function search_type() {
			...
			$result_datas = $result_datas2 = $this->db->select(array('module'=>'search'),'*',1000,'listorder ASC');
			foreach($result_datas as $_key=>$_value) {
			...
			//搜索header头中使用类型缓存
			setcache('search_model',$search_model,'search');
	...
	public function del_file() {
		$path = OLCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_template'.DIRECTORY_SEPARATOR;
		$files = glob($path.'*');
		...
		$path = OLCMS_PATH.'caches'.DIRECTORY_SEPARATOR.'caches_tpl_data'.DIRECTORY_SEPARATOR.'caches_data'.DIRECTORY_SEPARATOR;
		$files = glob($path.'*');
		if (is_array($files)) {
			foreach ($files as $f) {
				$dir = basename($f);
