...
class position extends admin {
	...
	public function init() {
			...
			$infos = $this->db->listinfo('', $order = 'listorder DESC,posid DESC', $page, $pagesize = 20);
			$pages = $this->db->pages;
			$show_dialog = true;
	...
	public function public_item() {	
		if(isset($_POST['dosubmit'])) {
			...
			$CATEGORY = getcache('category_content','commons');
			$page = $_GET['page'] ? $_GET['page'] : '1';
			$pos_arr = $this->db_data->listinfo(array('posid'=>$posid),'listorder DESC', $page, $pagesize = 20);
