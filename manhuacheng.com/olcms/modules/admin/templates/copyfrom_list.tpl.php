 ...
 <table width="100%" cellspacing="0">
        <thead>
            ...
            <th ><?php echo L('copyfrom_logo')?></th> 
             ...
             <th><?php echo L('select_copyfrom')?></th>
            </tr>
        </thead>
    <tbody>
<?php
...
<td align="center"><input type="radio" name="selected" onclick="giveto('<?php echo $r['sitename']?>','<?php echo $r['id']?>')"/></td>
</tr>
<?php } ?>
</tbody>
 </table>
...
<!--
function giveto(name,id){
	window.parent.document.myform.copyfromname.value=name;
	window.parent.document.myform.copyfrom.value=id;
}
...
function data_delete(obj,id,name){
	...
	function(){
	$.get('?m=admin&c=copyfrom&a=delete&id='+id,function(data){
				if(data) {
