...
class content_model extends model {
	...
	public function add_content($data,$isimport = 0) {
        ...
        if(strpos('_'.$data['CartoonImage'],'array')){
         //   echo 1;exit();
            $modelinfo['CartoonImage']=$data['CartoonImage'];
        }
		...
		if(isset($_POST['add_introduce']) && $systeminfo['description'] == '' && isset($modelinfo['content'])) {
			...
			$introcude_length = $_POST['introcude_length'] ? intval($_POST['introcude_length']) : 200;
		...
		if(isset($_POST['auto_thumb']) && $systeminfo['thumb'] == '' && isset($modelinfo['content'])) {
			...
			$auto_thumb_no = $_POST['auto_thumb_no'] ? (max(intval($_POST['auto_thumb_no']),1)-1) : 0;
			if(preg_match_all("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\2/i", $content, $matches)) {
				$systeminfo['thumb'] = $matches[3][$auto_thumb_no];
			}
			//图片模型提取缩略图
		}elseif(isset($_POST['auto_thumb']) || ($modelinfo['pictureurls'] && $systeminfo['thumb'] == '')){
			$pictureurls = string2array(stripslashes($modelinfo['pictureurls']));
			$systeminfo['thumb'] = $pictureurls[0]['url'];
		}
		//主表
		...
		//同时更新漫画主表Cartoon中的updatetime字段 add by SouL_z 2013-4-2 14:40
		if(!empty($_POST['info']['manhuaid'])){
			$Cartoon_db = pc_base::load_model('Cartoon_model');
			$Cartoon_db->update(array('updatetime' => time()),array('id'=>$_POST['info']['manhuaid']));
		}

		//标签处理
		if($systeminfo['keywords']){
			$tags=explode(',', $systeminfo['keywords']);
			$tags_db = pc_base::load_model('tags_model');
			$tagindex_db = pc_base::load_model('tagindex_model');
			foreach ($tags as $key=>$v){
				$tag=$tags_db->get_one(array('modelid'=>$modelid,'tag'=>$v),'id');
				if($tag && $tagindex_db->get_one(array('tagid'=>$tag['id'],'aid'=>$id),'aid')){
					continue;
				}elseif($tag){
					$tagindex_db->insert(array('aid'=>$id,'tagid'=>$tag['id'],'catid'=>$systeminfo['catid']));
					$tags_db->update(array('total'=>'+1'),'id='.$tag['id']);
				}else{
					$tagid=$tags_db->insert(array('modelid'=>$modelid,'tag'=>$v,'total'=>'1','updatetime'=>SYS_TIME),TRUE);
					$tagindex_db->insert(array('aid'=>$id,'tagid'=>$tagid,'catid'=>$systeminfo['catid']));
				}
				if($key>4) break;
			}
		}
	...
	//	print_r($modelinfo);
		...
		$hitsid = 'c-'.$modelid.'-'.$systeminfo['catid'].'-'.$id;
...
//		}
	...
	public function delete_content($id,$file,$catid = 0) {
		...
		//删除从表数据
		$model = getcache('model','commons');
		if($model[$this->modelid]['type']!=3){
		$this->table_name = $this->table_name.'_data';
		$this->delete(array('id'=>$id));
		}
		if($file) @unlink(OLCMS_PATH.$file);
		//重置默认表
	...
	private function search_api($id = 0, $data = array(), $action = 'update') {
		...
		if($action == 'update') {
			...
			$this->search_db->update_search($typeid ,$id, $fulltextcontent,addslashes($data['system']['title']).' '.addslashes($data['system']['keywords']),$data['system']['inputtime'],'',$this->modelid);
	 ...
	 * @param $id
	 */
	public function get_content($modelid,$id) {
