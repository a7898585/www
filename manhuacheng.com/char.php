...
if(ord($letter)>64&&ord($letter)<91||ord($letter)==48){
		
		$page=intval($page)>-1?intval($page):0;
		$s=$page*20;	

		$db = new db($db_config);
		$numsql="select count(id) as n from mh_cartoon where CharIndex='$letter'";
		$nums=$db->GetOne($numsql);
//		拼凑sql语句
		$strsql="select id,CartoonName,Author,UpdateTime,CartoonState,CartoonImage from mh_cartoon where CharIndex='$letter' limit $s,20";
